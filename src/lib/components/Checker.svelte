<script lang="ts">
  ...
  const labels: Record<string, string> = { invisible: 'unsichtbare Zeichen', controls: 'Steuerzeichen', bidi: 'Richtungssteuerungen', tags: 'versteckte Tag-Zeichen', variation: 'Variationsselektoren', spaces: 'ungewöhnliche Leerzeichen', typography: 'typografische Ersetzungen', confusables: 'schriftsystemübergreifende Lookalikes' };
    ...
    <section class="card workspace-panel bg-base-100 shadow-sm" aria-labelledby="comparison-title">
      <div class="card-body">
        <div class="panel-heading">
          ...
          <p class="label panel-length">{report.findings.length} Änderungen</p>
        </div>
              ...
              <tbody>
                {#each comparisonEntries as entry}
                  <tr>
                    ...
                    <td>{entry.action === 'strip' ? 'entfernt' : 'ersetzt'}<span class="comparison-category">{labels[entry.category]}</span></td>
                  </tr>
                {/each}
              </tbody>
            </table>
          </div>
          {#if comparisonIsTruncated}<p class="label comparison-note">Es werden die ersten {maxComparisonEntries} von {report.findings.length} Änderungen angezeigt.</p>{/if}
        {:else}
