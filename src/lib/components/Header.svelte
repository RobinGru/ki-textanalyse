<script lang="ts">
  ...
  function toggleTheme() {
    ...
    localStorage.setItem('ki-textanalyse-theme', theme);
...
<header class="site-header">
  <nav class="navbar site-nav" aria-label="Hauptnavigation">
    <a class="site-brand" href={`${base}/`} aria-label="KI-Textanalyse – Startseite">
      <span class="brand-mark" aria-hidden="true"><ShieldCheck size={20} strokeWidth={2.25} /></span>
      <span class="brand-copy"><strong>KI-Textanalyse</strong><span>Wasserzeichen &amp; Heuristiken <small>by RobinGru</small></span></span>
    </a>
    <div class="site-links">
      {#each links as link}
        ...
        >{link.label}</a>
      {/each}
      <a class="btn btn-ghost btn-sm nav-link" href="https://github.com/RobinGru/ki-textanalyse" aria-label="KI-Textanalyse auf GitHub öffnen">
